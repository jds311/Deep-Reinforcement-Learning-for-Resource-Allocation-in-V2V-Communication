...
with g.as_default():
    ...
    w_Value = tf.Variable(tf.truncated_normal([n_hidden_3, 1], stddev=0.1))
    w_Advantage = tf.Variable(tf.truncated_normal([n_hidden_3, n_output], stddev=0.1))
    ...
    b_Value = tf.Variable(tf.truncated_normal([1], stddev=0.1))
    b_Advantage = tf.Variable(tf.truncated_normal([n_output], stddev=0.1))
    ...
    Value = (tf.add(tf.matmul(layer_3_b, w_Value), b_Value))
    Advantage = (tf.add(tf.matmul(layer_3_b, w_Advantage), b_Advantage))

    #Then combine them together to get our final Q-values.
    y = Value + (Advantage - tf.reduce_mean(Advantage, axis=1,keep_dims=True))	  # Q = V(s) + A(s,a)
    ...
    w_Value_p = tf.Variable(tf.truncated_normal([n_hidden_3, 1], stddev=0.1))
    w_Advantage_p = tf.Variable(tf.truncated_normal([n_hidden_3, n_output], stddev=0.1))
    ...
    b_Value_p = tf.Variable(tf.truncated_normal([1], stddev=0.1))
    b_Advantage_p = tf.Variable(tf.truncated_normal([n_output], stddev=0.1))
    ...
    Value_p = (tf.add(tf.matmul(layer_3_p_b, w_Value_p), b_Value_p))
    Advantage_p = (tf.add(tf.matmul(layer_3_p_b, w_Advantage_p), b_Advantage_p))

    #Then combine them together to get our final Q-values.
    y_p = Value_p + (Advantage_p - tf.reduce_mean(Advantage_p, axis=1,keep_dims=True))	  # Q = V(s) + A(s,a)

    #y_p = tf.nn.relu(tf.add(tf.matmul(layer_3_p_b, w_4_p), b_4_p))		#### check it
...
def update_target_q_network(sess):
    ...
    sess.run(w_Value_p.assign(sess.run(w_Value)))
    sess.run(w_Advantage_p.assign(sess.run(w_Advantage)))
    
    sess.run(b_1_p.assign(sess.run(b_1)))
    sess.run(b_2_p.assign(sess.run(b_2)))
    sess.run(b_3_p.assign(sess.run(b_3)))
    sess.run(b_Value_p.assign(sess.run(b_Value)))
    sess.run(b_Advantage_p.assign(sess.run(b_Advantage)))
...
if IS_TRAIN:
    for i_episode in range(n_episode):
        ...
        print('reward:[',i_episode,']:  ',sum(record_reward[i_episode*100 : ((i_episode+1)*100)]))
        reward_plot.append(sum(record_reward[i_episode*100 : ((i_episode+1)*100)]))
        ...
        if (i_episode+1) % 50 == 0:
            ...
            plt.ylim(20, 100)
            plt.grid()
            plt.savefig('reward_output/save_'+str(i_episode)+'.png')
            #plt.show()

            print('Training Done. Saving models...')
            for i in range(n_veh):
            	for j in range(n_neighbor):
                    model_path = label +'_'+ str(i_episode) + '/agent_' + str(i * n_neighbor + j)
                    save_models(sesses[i * n_neighbor + j], model_path)

            current_dir = os.path.dirname(os.path.realpath(__file__))
            reward_path = os.path.join(current_dir, "model/" + label+'_'+str(i_episode) + '/reward.mat')
            scipy.io.savemat(reward_path, {'reward': record_reward})

            record_loss_ = np.asarray(record_loss).reshape((-1, n_veh*n_neighbor))
            loss_path = os.path.join(current_dir, "model/" + label+'_'+ str(i_episode) + '/train_loss.mat')
            scipy.io.savemat(loss_path, {'train_loss': record_loss_})
