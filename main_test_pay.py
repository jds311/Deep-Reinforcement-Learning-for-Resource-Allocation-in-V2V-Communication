...
Original file is located at
    https://colab.research.google.com/drive/1ISp0dLhiTgQ9BFieueeOKiR7hrAP_nXF
"""

#sys.path.insert(0,'/content/drive/My Drive/WC_Project_3_11/MARLspectrumSharingV2X-master')
...
V2I_rate_payload = np.zeros([4,6])
pr_V2V_payload = np.zeros([4,6])

for pay in range(6):
    env.demand_size = 8480
    env.demand_size = (pay+1) * env.demand_size
    ...
    if IS_TEST:
        ...
        initial_payload = env.demand_size
        ...
        V2I_rate_payload[0,pay] = np.average(V2I_rate_list)
        pr_V2V_payload[0,pay] = np.average(V2V_success_list)

        V2I_rate_payload[1,pay] = np.average(V2I_rate_list_sarl)
        pr_V2V_payload[1,pay] = np.average(V2V_success_list_sarl)

        V2I_rate_payload[2,pay] = np.average(V2I_rate_list_rand)
        pr_V2V_payload[2,pay] = np.average(V2V_success_list_rand)

        V2I_rate_payload[3,pay] = np.average(V2I_rate_list_dpra)
        pr_V2V_payload[3,pay] = np.average(V2V_success_list_dpra)
        ...
        '''
        plt.plot(V2I_rate_payload[0], [1,2,3,4,5,6],  label='MARL')
        plt.plot(V2I_rate_payload[1], [1,2,3,4,5,6],  label='SARL')
        plt.plot(V2I_rate_payload[2], [1,2,3,4,5,6],  label='Random')
        plt.plot(V2I_rate_payload[3], [1,2,3,4,5,6],  label='DPRA')
        '''
    ...
    # The name "DPRA" is used for historical reasons. Not really the case...
        ...
        with open("Data.txt", "a") as f:
            f.write('############################ payload, ' + str(initial_payload) + '############################\n')
            f.write('-------- marl, ' + label + '------\n')
...
plt.plot(V2I_rate_payload[0],  label='MARL')
plt.plot(V2I_rate_payload[1],  label='SARL')
plt.plot(V2I_rate_payload[2],  label='Random')
plt.plot(V2I_rate_payload[3],  label='DPRA')
plt.xlabel("N*payload")
plt.ylabel("Mbps")
plt.xlim(0, 5)
plt.ylim(25, 45)
plt.legend()
plt.grid()
plt.savefig('test_output/V2I_'+str(pay)+'.png')

plt.plot(pr_V2V_payload[0],  label='MARL')
plt.plot(pr_V2V_payload[1],  label='SARL')
plt.plot(pr_V2V_payload[2],  label='Random')
plt.plot(pr_V2V_payload[3],  label='DPRA')
plt.xlabel("N*payload")
plt.ylabel("V2V payload")
plt.xlim(0, 5)
plt.ylim(0.5, 1.1)
plt.legend()
plt.grid()
plt.savefig('test_output/V2V_'+str(pay)+'.png')
V2I_path = os.path.join(current_dir, "test_output/V2I.mat")
scipy.io.savemat(V2I_path, {'V2I_rate_payload': V2I_rate_payload})
V2V_path = os.path.join(current_dir, "test_output/V2V.mat")
scipy.io.savemat(V2V_path, {'pr_V2V_payload': pr_V2V_payload})
